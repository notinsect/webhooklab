...
async function handleWebhookIngestion(
  ...
  // 2. Enforce Content-Length Header check if present
  ...
  if (rawBody) {
    if (contentType.includes("application/json")) {
      try {
        ...
        // Malformed JSON: store rawBody string safely without throwing
        parsedBody = null;
      }
  ...
  // 7. Enforce Retention Policy (Keep latest 100 requests per endpoint)
  try {
    const subquery = db
